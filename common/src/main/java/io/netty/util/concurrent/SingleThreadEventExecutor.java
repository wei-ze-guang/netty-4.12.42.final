...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
    ...
    protected boolean runAllTasks(long timeoutNanos) {
        /**
         * 轮询任务队列中的所有任务，并通过 Runnable 运行它们。run（） 方法。此方法停止运行任务队列中的任务，如果运行时间超过 timeoutNanos 则返回。
         */
        fetchFromScheduledTaskQueue();
    ...
    public void execute(Runnable task) {
        log.info("[这里是启动一个EventLoop的最重要的方法。这里的话如果是这个EventLoop已经开启的话就是往普通任务队列里面加任务，加入任务之后如果Event没开启的是也直接开启EventLoop]");
        if (task == null) {
            throw new NullPointerException("task");
        }
    ...
    private void doStartThread() {
        assert thread == null;
        log.info("[NioEventLoop 正式启动一个NioEventLoop 这个executor 是一个外部专门启动EventLoop的类 这里向循环时间提交了一个任务，这个任务的run方法就是运行事件循环里面的 run方法]");
