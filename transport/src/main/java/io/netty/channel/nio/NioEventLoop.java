...
public final class NioEventLoop extends SingleThreadEventLoop {
    ...
    //这种方法好处是只有真正调用的时候才生效，比如传进去参数不会自动调用只有真正get，方法才会执行
        ...
        try {
            ...
            for (;;) {
                ...
                if (selectedKeys != 0 || oldWakenUp || wakenUp.get() || hasTasks() || hasScheduledTasks()) {
                    ...
                    // - waken up by user, or  //  外部wakeup
                    // - the task queue has a pending task.   有普通任何任务
                    // - a scheduled task is ready for processing  或者定时任务  需要跳出循环
                    break;
                }
